...
# ==============================================================================
# 3. FASE 2: REFINAMENTO LOCAL (Pattern Search) - BLINDADA
# ==============================================================================
def refinar_com_pattern_search(config, params_iniciais, objetivo_escolhido):
    ...
    print("(Pressione Ctrl+C a qualquer momento para salvar o melhor resultado atual)")
        ...
        while True:
            ...
            for param_info in lista_de_parametros:
                ...
                for val_teste in candidatos:
                    ...
                    if melhorou:
                        print(f"  [Refinamento] Melhoria em {nome}: {val_teste} -> {res}")
            ...
            if not houve_melhoria:
                print(f"  Refinamento concluído na iteração {iteracao}.")
                break
...
def gerar_relatorio_arquivo(dados):
    ...
    nome_arquivo = f"relatorio_{nome_modelo}_{data_hora}.txt"
    
    val_inicial = dados['resultado_fase1']
    val_final = dados['resultado_final']
    
    # Evita erro se Fase 1 for None (parou muito cedo)
...
if __name__ == "__main__":
    ...
    # Variáveis globais de resultado (inicializadas vazias)
    melhor_fase1 = None
    params_fase1 = {}
    ...
    try:
        ...
        # --- LÓGICA DE ESCOLHA (Menu ou Flag) ---
        ...
        if objetivo_cliente is None:
            print("\n" + "="*60)
            print("              CONFIGURADOR DE OTIMIZAÇÃO")
            print("="*60)
            ...
            while True:
                try:
                    ...
                    # Se der erro aqui, não tem relatório pois nem começou
        ...
        # Executa Optuna com proteção para KeyboardInterrupt
        try:
        ...
        # Recupera o melhor da Fase 1 (se houver)
        if len(study.trials) > 0:
            ...
            # Se parou antes do primeiro trial, usa valores iniciais
            params_fase1 = {p['nome']: p['valor_inicial'] for p in CONFIG_GLOBAL['parametros']}
            melhor_fase1 = 0

        # FASE 2: PATTERN SEARCH (Se não foi interrompido ou se quer tentar refinar o parcial)
        if status_execucao != "INTERROMPIDO":
            ...
            # Se parou na Fase 1, o resultado da Fase 1 é o Final
            params_finais = params_fase1
            resultado_final = melhor_fase1

    except KeyboardInterrupt:
        # Captura interrupção geral (caso aconteça fora dos loops tratados)
        print("\n\n⚠️  PARADA TOTAL SOLICITADA PELO USUÁRIO.")
        status_execucao = "INTERROMPIDO"
        if params_finais == {}: # Se parou na fase 2 e retornou vazio
            params_finais = params_fase1
            resultado_final = melhor_fase1
        ...
        # ======================================================================
        # BLOCO FINALLY: ISSO RODA SEMPRE, MESMO SE VOCÊ PARAR O PROGRAMA
        # ======================================================================
        tempo_gasto = time.time() - inicio_total

        # Se ainda não temos resultado final (ex: parou no meio da Fase 2), pega o da Fase 1
